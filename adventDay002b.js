...
rawInput.forEach(input => {
    let { groups } = /(?<min>\d+)-(?<max>\d+) (?<req>[a-z]): (?<pw>[a-z]+)/.exec(input);
    let rawPassword = groups.pw.split("");
    let firstPos = groups.min - 1;
    let secondPos = groups.max - 1;

    if ((rawPassword[firstPos] === groups.req || rawPassword[secondPos] === groups.req) &&
        !(rawPassword[firstPos] === groups.req && rawPassword[secondPos] === groups.req)) {
        validPasswords++;
    };
});
